...
class LinkJointGraph {
 public:
  ...
  additional Joints may be added during Forest building, so you may get a
  ...
  /** After the Forest is built, returns the sequence of Links from the given
  one to World, inclusive of both. Cost is O(ℓ) where ℓ is the give Link's level
  in the SpanningForest.
  ...
  std::vector<std::set<LinkIndex>> FindSubgraphsOfWeldedLinksNoForest() const;
