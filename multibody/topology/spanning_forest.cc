...
std::vector<MobodIndex> SpanningForest::FindPathToWorld(
...
std::pair<int, int> SpanningForest::FindOutboardVelocities(
    MobodIndex mobod_index) const {
  const auto [first_v, nv] = FindSubtreeVelocities(mobod_index);
  const Mobod& mobod = mobods(mobod_index);
  return std::make_pair(first_v + mobod.nv(), nv - mobod.nv());
}

std::pair<int, int> SpanningForest::FindSubtreeVelocities(
    MobodIndex mobod_index) const {
  const Mobod& mobod = mobods(mobod_index);
...
void SpanningForest::ChooseForestTopology() {
  ...
  policy, and then extend from there to build the whole tree. Then we add each
...
const SpanningForest::Mobod& SpanningForest::AddNewMobod(
  ...
  if (!tree_index.is_valid()) {
    ...
    // A new tree has level 1, so forest has at least level 2 now.
