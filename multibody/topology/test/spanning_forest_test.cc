...
GTEST_TEST(SpanningForest, MasslessLinksChangeLoopBreaking) {
  ...
  EXPECT_EQ(ssize(graph.joints()), 7);
  EXPECT_TRUE(graph.links(BodyIndex(7)).is_shadow());
  EXPECT_EQ(graph.links(BodyIndex(7)).primary_link(), BodyIndex(4));

  EXPECT_EQ(ssize(forest.trees()), 2);
  EXPECT_EQ(forest.trees()[0].num_mobods(), 4);
  EXPECT_EQ(forest.trees()[1].num_mobods(), 3);
  EXPECT_EQ(forest.mobods(MobodIndex(4)).link(), BodyIndex(7));
  ...
  EXPECT_EQ(ssize(graph.joints()), 7);
  EXPECT_TRUE(graph.links(BodyIndex(7)).is_shadow());
  EXPECT_EQ(graph.links(BodyIndex(7)).primary_link(), BodyIndex(6));

  EXPECT_EQ(ssize(forest.trees()), 2);
  EXPECT_EQ(forest.trees()[0].num_mobods(), 5);
  EXPECT_EQ(forest.trees()[1].num_mobods(), 2);
  EXPECT_EQ(forest.mobods(MobodIndex(5)).link(), BodyIndex(7));
}
