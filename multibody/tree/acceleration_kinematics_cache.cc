...
template <typename T>
void AccelerationKinematicsCache<T>::InitializeToNaN() {
  for (SpatialAcceleration<T>& acc : A_WB_pool_) acc.SetNaN();
  for (SpatialAcceleration<T>& acc : A_WL_pool_) acc.SetNaN();
  for (T& vdot : vdot_) vdot = std::numeric_limits<double>::quiet_NaN();
}
