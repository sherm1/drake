...
template <class T> class LinkFrame;
template <class T> class RigidLink;
...
/// fixed pose `X_BF`, where B is the LinkFrame associated with body B.
...
class FixedOffsetFrame final : public Frame<T> {
 public:
  ...
  /// LinkFrame B of the given Link, which serves as F's parent frame.
  ...
  /// @param[in] bodyB The body whose LinkFrame B is to be F's parent frame.
