...
class FrameBodyPoseCache {
 public:
  ...
  const math::RigidTransform<T>& get_X_BL(LinkIndex index) const {
    // This method must be very fast in Release.
    DRAKE_ASSERT(0 <= index && index < ssize(X_BL_pool_));
    return X_BL_pool_[index];
  }

  bool is_X_BL_identity(LinkIndex index) const {
    // This method must be very fast in Release.
    DRAKE_ASSERT(0 <= index && index < ssize(is_X_BL_identity_));
    return static_cast<bool>(is_X_BL_identity_[index]);
  }

  const SpatialInertia<T>& get_M_LLo_L(LinkIndex index) const {
    DRAKE_ASSERT(0 <= index && index < ssize(M_LLo_L_pool_));
    return M_LLo_L_pool_[index];
  }
  ...
  void SetX_BL(LinkIndex index, const math::RigidTransform<T>& X_BL) {
    // This method is only called when parameters change.
    DRAKE_DEMAND(0 <= index && index < ssize(X_BL_pool_));
    X_BL_pool_[index] = X_BL;
    if constexpr (scalar_predicate<T>::is_bool) {
      is_X_BL_identity_[index] = X_BL.IsExactlyIdentity();
    } else {
      is_X_BL_identity_[index] = static_cast<uint8_t>(false);
    }
  }

  void SetM_LLo_L(LinkIndex index, const SpatialInertia<T>& M_LLo_L) {
    // This method is only called when parameters change.
    DRAKE_DEMAND(0 <= index && index < ssize(M_LLo_L_pool_));
    M_LLo_L_pool_[index] = M_LLo_L;
  }
 ...
 private:
  ...
  // These are indexed by LinkIndex.
