...
bool MultibodyTree<T>::HasUniqueFreeBaseBodyImpl(
    ModelInstanceIndex model_instance) const {
  std::optional<LinkIndex> base_body_index =
      MaybeGetUniqueBaseBodyIndex(model_instance);
  return base_body_index.has_value() &&
         owned_links_[base_body_index.value()]->is_floating();
}

template <typename T>
const Link<T>& MultibodyTree<T>::GetUniqueFreeBaseBodyOrThrowImpl(
    ModelInstanceIndex model_instance) const {
  std::optional<LinkIndex> base_body_index =
      MaybeGetUniqueBaseBodyIndex(model_instance);
  ...
  if (!owned_links_[base_body_index.value()]->is_floating()) {
    throw std::logic_error("Model " +
                           instance_index_to_name_.at(model_instance) +
                           " has a unique base body, but it is not free.");
  }
  return *owned_links_[base_body_index.value()];
}

namespace {

// Given an ElementIndex type (e.g., LinkIndex) as a template argument, returns
// the name of the corresponding class (e.g., "Body").
template <typename ElementIndex>
std::string_view GetElementClassname() {
  if constexpr (std::is_same_v<ElementIndex, LinkIndex>) {
    return "Body";
  }
...
void MultibodyTree<T>::CreateJointImplementations() {
  ...
  for (LinkIndex link_index(1); link_index < num_links(); ++link_index) {
    const Link<T>& body = get_link(link_index);
    const BodyTopology& body_topology = get_topology().get_link(body.index());
...
void MultibodyTree<T>::FinalizeInternals() {
  ...
  // setup.
  for (const auto& body : owned_links_) {
  ...
  for (int i = 0; i < num_joints(); ++i) {
    auto& joint = owned_joints_[i];
    const Link<T>& body = joint->child_body();
    if (body.is_floating()) {
...
void MultibodyTree<T>::CreateBodyNode(MobodIndex mobod_index) {
  const BodyNodeTopology& node_topology =
      topology_.get_body_node(mobod_index);
  const LinkIndex link_index = node_topology.body;

  const Link<T>* body = owned_links_[node_topology.body].get();
...
void MultibodyTree<T>::SetDefaultFreeBodyPose(
    const Link<T>& body, const RigidTransform<double>& X_WB) {
...
void MultibodyTree<T>::SetFreeBodyPoseOrThrow(
    const Link<T>& body, const RigidTransform<T>& X_WB,
    systems::Context<T>* context) const {
  DRAKE_MBT_THROW_IF_NOT_FINALIZED();
...
void MultibodyTree<T>::SetFreeBodyRandomPositionDistributionOrThrow(
    const Link<T>& body, const Vector3<symbolic::Expression>& position) {
  DRAKE_MBT_THROW_IF_NOT_FINALIZED();
...
void MultibodyTree<T>::CalcAllBodySpatialVelocitiesInWorld(
  ...
  if (static_cast<int>(V_WB->size()) != num_links()) {
    V_WB->resize(num_links(), SpatialVelocity<T>::Zero());
  }
  const VelocityKinematicsCache<T>& vc = EvalVelocityKinematics(context);
  for (LinkIndex link_index(0); link_index < num_links(); ++link_index) {
    const MobodIndex mobod_index = get_link(link_index).mobod_index();
    V_WB->at(link_index) = vc.get_V_WB(mobod_index);
  }
}
...
// Result is indexed by MobodIndex, not LinkIndex.
template <typename T>
...
// The result is in MobodIndex order.
template <typename T>
...
VectorX<T> MultibodyTree<T>::CalcInverseDynamics(
  ...
  std::vector<SpatialAcceleration<T>> A_WB(topology_.num_mobods());
  std::vector<SpatialForce<T>> F_BMo_W(topology_.num_mobods());
...
RigidTransform<T> MultibodyTree<T>::CalcRelativeTransform(
  ...
  const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
  const Link<T>& A = frame_F.body();
  const Link<T>& B = frame_G.body();
...
SpatialInertia<T> MultibodyTree<T>::CalcSpatialInertia(
  ...
  // Check if there are repeated LinkIndex in body_indexes by converting the
  // vector to a set (to eliminate duplicates) and see if their sizes differ.
  const std::set<LinkIndex> without_duplicate_links(
      link_indexes.begin(), link_indexes.end());
  if (link_indexes.size() != without_duplicate_links.size()) {
      throw std::logic_error(
  ...
  for (LinkIndex link_index : link_indexes) {
    if (link_index == 0) continue;  // No contribution from the world body.
...
SpatialMomentum<T> MultibodyTree<T>::CalcSpatialMomentumInWorldAboutPoint(
    ...
    const Vector3<T>& p_WoP_W) const {
  // Assemble a list of LinkIndex.
  std::vector<LinkIndex> body_indexes;
  for (auto model_instance : model_instances) {
    ...
    const std::vector<LinkIndex> body_index_in_instance =
        GetBodyIndices(model_instance);
    for (LinkIndex link_index : body_index_in_instance)
      body_indexes.push_back(link_index);
  }
...
SpatialMomentum<T> MultibodyTree<T>::CalcBodiesSpatialMomentumInWorldAboutWo(
  ...
  for (LinkIndex link_index : body_indexes) {
    if (link_index == 0) continue;  // No contribution from the world body.
...
void MultibodyTree<T>::CalcAllBodyBiasSpatialAccelerationsInWorld(
  ...
  DRAKE_THROW_UNLESS(ssize(*AsBias_WB_all) == topology_.num_mobods());
...
SpatialAcceleration<T> MultibodyTree<T>::CalcBiasSpatialAcceleration(
  ...
  std::vector<SpatialAcceleration<T>> AsBias_WB_all(topology_.num_mobods());
  ...
  const Link<T>& body_B = frame_B.body();
  ...
  const Link<T>& body_A = frame_A.body();
...
SpatialAcceleration<T> MultibodyTree<T>::ShiftSpatialAccelerationInWorld(
  ...
  const Link<T>& body_A = frame_B.body();
...
void MultibodyTree<T>::CalcJacobianAngularAndOrTranslationalVelocityInWorld(
  ...
  const Link<T>& body_F = frame_F.body();
...
void MultibodyTree<T>::ThrowDefaultMassInertiaError() const {
  ...
  // multiple sets of links that are welded to each other. Each set of links
  // consists of a parent link (the first entry in the set) and thereafter
  // child links that are welded to the parent.
  ...
  const size_t number_of_sets = welded_links_list.size();
  ...
  for (size_t i = 1;  i < number_of_sets;  ++i) {
    ...
    const std::set<LinkIndex>& welded_link = welded_links_list[i];
    const LinkIndex parent_link_index = *welded_link.begin();
    const LinkTopology& parent_link_topology =
        tree_topology.get_link(parent_link_index);
    const MobilizerIndex& parent_mobilizer_index =
        parent_link_topology.inboard_mobilizer;
    const Mobilizer<T>& parent_mobilizer =
        get_mobilizer(parent_mobilizer_index);

    // Check previous assumptions.
    const Link<T>& parent_link = get_link(parent_link_index);
    DRAKE_ASSERT(parent_link_index == parent_link.index());
    DRAKE_ASSERT(parent_link_index != world_index());

    // Determine whether this set of welded links is a most distal leaf in
    ...
    const MobodIndex parent_mobod_index = parent_link_topology.mobod_index;
    ...
    if (is_composite_body_distal_leaf_in_tree) {
      ...
      const bool has_no_mass = CalcTotalDefaultMass(welded_link) == 0;
      ...
      if (parent_mobilizer.can_rotate()) {
        ...
        if (IsAnyDefaultRotationalInertiaNaN(welded_link)) {
          const std::string msg = fmt::format(
        ...
        if (has_no_mass && AreAllDefaultRotationalInertiaZero(welded_link)) {
          const std::string msg = fmt::format(
...
double MultibodyTree<T>::CalcTotalDefaultMass(
  ...
  double total_mass = 0;
  for (LinkIndex link_index : body_indexes) {
    const Link<T>& body_B = get_link(link_index);
...
template <typename T>
std::optional<LinkIndex> MultibodyTree<T>::MaybeGetUniqueBaseBodyIndex(
    ModelInstanceIndex model_instance) const {
  DRAKE_THROW_UNLESS(model_instance < instance_name_to_index_.size());
  ...
  for (const auto& body : owned_links_) {
    if (body->model_instance() == model_instance &&
        (topology_.get_link(body->index()).parent_body == world_index())) {
      if (base_body_index.has_value()) {
