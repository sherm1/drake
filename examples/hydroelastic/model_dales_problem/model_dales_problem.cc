...
DEFINE_double(mbp_discrete_update_period, 1./64./2,  // match visualization
...
DEFINE_double(simulation_sec, 3./64./2, "Number of seconds to simulate. [s].");
// The following flags are only in effect in continuous mode.
DEFINE_double(accuracy, 1.0e-3, "The integration accuracy.");
DEFINE_double(max_time_step, 1./64./2,  // match discrete update
...
int DoMain() {
  ...
  const std::vector<multibody::ModelInstanceIndex> schunk_instance =
      parser.AddModelsFromUrl(
          "package://drake_models/wsg_50_description/sdf/"
          "schunk_wsg_50_hydro_bubble.sdf");
  const std::vector<multibody::ModelInstanceIndex> spatula_instance =
      parser.AddModelsFromUrl(
      "package://drake/examples/hydroelastic/model_dales_problem/models/"
      "spatula.sdf");
  ...
  if (true) {
    ...
    const auto filter =
        builder.AddNamedSystem<systems::FirstOrderLowPassFilter>(
            "FOLPF", 10., 6);
    builder.Connect(
        plant.get_generalized_contact_forces_output_port(spatula_instance[0]),
  ...
  */
  const auto& demux =  // chop up state
      *builder.AddNamedSystem<systems::Demultiplexer>("demux",
                              std::vector<int>{2, 7, 2, 6});  // joints, spatula
  ...
  builder.Connect(demux.get_output_port(2),  // joint velocities
  ...
  std::ofstream dot_file("/tmp/model_dales_problem.dot");
  dot_file << diagram->GetGraphvizString(
      {},
      std::map({std::pair<std::string, std::string>{"plant/split", "I/O"}}));
  dot_file.close();
  ...
  std::cout << "ComputeContactSurfaces x "
            << plant.compute_contact_surfaces_count() << "\n";

  std::cout << "ComputeContactSurfacesWithFallback x "
            << plant.compute_contact_surfaces_with_fallback_count() << "\n";
  return 0;
}
