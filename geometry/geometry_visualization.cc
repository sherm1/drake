...
class LcmInitSystem : public systems::LeafSystem<double> {
 public:
  ...
  LcmInitSystem(const SceneGraph<double>* scene_graph,
    ...
    // geometry, once geometry is changeable at runtime.
...
namespace {
void VisualizationBuilder(systems::DiagramBuilder<double>* builder,
  ...
  LcmPublisherSystem* publisher =
      builder->template AddSystem<LcmPublisherSystem>(
          "DRAKE_VIEWER_DRAW",
  std::make_unique<Serializer<drake::lcmt_viewer_draw>>(), &lcm_system->lcm());
...
void ConnectVisualization(const SceneGraph<double>& scene_graph,
  ...
  VisualizationBuilder(builder, scene_graph, lcm);
}
...
void AddVisualization(systems::DiagramBuilder<double>* builder,
  ...
  // Create a DrakeLcm internally.
  VisualizationBuilder(builder, scene_graph, nullptr);

  // Connect systems to be visualized to scene graph input.
  builder->Connect(output_to_visualize,
